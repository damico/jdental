...
public class FormGenerator {
	...
	public String displayForm() throws UnsupportedEncodingException {
		...
		for(int i=0; i<ffList.size(); i++){
			...
			if(ffList.get(i).getType().equalsIgnoreCase("hidden")){
				elements.append("<input type='"+ffList.get(i).getType()+"' name='"+getName(label)+"' value='"+value+"'><br>\n");
			}else if(ffList.get(i).getType().equalsIgnoreCase("submit")) {
				elements.append("<input type='"+ffList.get(i).getType()+"' name='"+getName(label)+"' value='"+value+"' class='botao'><br>\n");
			}else{	
				elements.append("<label class='title'>"+label+":</label> <right><input type='"+ffList.get(i).getType()+"' name='"+getName(label)+"' value='"+value+"' class='campos'></right><br>\n");
			}
		}
